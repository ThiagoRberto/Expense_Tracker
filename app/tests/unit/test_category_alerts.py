...
class TestCalculateCategoryTotals:
    ...
    def test_single_expense_single_category(self):
        result = calculate_category_totals([ExpenseData(300, 1, 1, 2025, "alimentação")])
        assert result == {"alimentação": 300.0}

    def test_installment_expense_contributes_monthly_fraction(self):
        result = calculate_category_totals([ExpenseData(1200, 4, 1, 2025, "transporte")])
    ...
    def test_multiple_expenses_different_categories(self):
        expenses = [
            ExpenseData(500, 1, 1, 2025, "alimentação"),
            ExpenseData(200, 1, 1, 2025, "transporte"),
        ]
        result = calculate_category_totals(expenses)
        assert result == {"alimentação": 500.0, "transporte": 200.0}

    def test_default_category_geral(self):
        # category omitida → default "geral" (continua sendo o único default)
        result = calculate_category_totals([ExpenseData(100, 1, 1, 2025)])
        assert "geral" in result

    def test_installment_zero_treated_as_one(self):
        result = calculate_category_totals([ExpenseData(300, 0, 1, 2025, "lazer")])
        assert result == {"lazer": 300.0}

    def test_three_categories(self):
        expenses = [
            ExpenseData(600, 2, 1, 2025, "alimentação"),   # 300
            ExpenseData(400, 1, 1, 2025, "transporte"),    # 400
            ExpenseData(900, 3, 1, 2025, "lazer"),          # 300
        ]
        result = calculate_category_totals(expenses)

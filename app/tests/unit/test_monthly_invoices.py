...
from services.financial_service import (
    project_monthly_invoices, InstallmentPurchase, MonthlyInvoice,
)
...
class TestProjectMonthlyInvoices:
    ...
    def test_single_purchase_single_installment_in_first_month(self):
        # compra à vista de 500 começando no mês de referência
        purchases = [InstallmentPurchase(500.0, 1, 6, 2025)]
        result = project_monthly_invoices(purchases, 6, 2025, 3)
        assert result == [
    ...
    def test_installments_spread_across_months(self):
        ...
        purchases = [InstallmentPurchase(300.0, 3, 6, 2025)]
        result = project_monthly_invoices(purchases, 6, 2025, 3)
        assert [inv.total for inv in result] == [100.0, 100.0, 100.0]

    def test_multiple_purchases_sum_in_same_month(self):
        # duas compras cuja parcela cai em junho devem somar
        purchases = [
            InstallmentPurchase(300.0, 3, 6, 2025),   # 100 em jun/jul/ago
            InstallmentPurchase(200.0, 1, 6, 2025),   # 200 em junho
        ]
        result = project_monthly_invoices(purchases, 6, 2025, 3)
    ...
    def test_every_window_month_present_even_without_installments(self):
        # nenhuma compra → todos os meses zerados, mas presentes
    ...
    def test_installments_beyond_window_are_ignored(self):
        ...
        purchases = [InstallmentPurchase(1200.0, 12, 1, 2025)]
        result = project_monthly_invoices(purchases, 1, 2025, 2)
        assert len(result) == 2
    ...
    def test_past_installments_before_reference_are_ignored(self):
        ...
        purchases = [InstallmentPurchase(400.0, 4, 1, 2025)]
        result = project_monthly_invoices(purchases, 3, 2025, 4)
    ...
    def test_window_crosses_year_boundary(self):
        ...
        purchases = [InstallmentPurchase(300.0, 3, 11, 2025)]
        result = project_monthly_invoices(purchases, 11, 2025, 3)
        assert [(inv.month, inv.year) for inv in result] == [
    ...
    def test_rounding_correction_aggregated(self):
        ...
        purchases = [InstallmentPurchase(10.0, 3, 1, 2025)]
        result = project_monthly_invoices(purchases, 1, 2025, 3)
    ...
    def test_months_ahead_one_returns_only_reference_month(self):
        purchases = [InstallmentPurchase(300.0, 3, 6, 2025)]
        result = project_monthly_invoices(purchases, 6, 2025, 1)
    ...
    def test_invalid_installments_propagates_from_project_installments(self):
        # política estrita herdada de project_installments
        purchases = [InstallmentPurchase(300.0, 0, 6, 2025)]
        with pytest.raises(ValueError, match="Installments must be at least 1"):
            project_monthly_invoices(purchases, 6, 2025, 3)
